...
class ViewModeSwitcher:
    def __init__(self):
        ...
        try:
            ...
            if not (0 <= self.current_mode <= 1):  # 现在只有0和1两种模式
        ...
        rospy.loginfo(f"View mode switcher initialized with mode {self.current_mode} (0=left, 1=right)")
    ...
    def handle_switch_view(self, req):
        ...
        if mode > 1:
            mode = 1
            rospy.logwarn(f"深度视图已禁用，切换为右视图")
        ...
        if 0 <= mode <= 1:
            ...
            if self.current_mode != mode:
                ...
                rospy.loginfo(f"Switched from mode {old_mode} to mode {self.current_mode} (0=left, 1=right)")
            ...
            response.message = "Invalid view mode. Use 0 (left) or 1 (right)"
            return response
    ...
    def run(self):
        ...
        rospy.loginfo(f"Starting view mode switcher with mode {self.current_mode} (0=left, 1=right)")
        
        while not rospy.is_shutdown():
            # 只在模式变化时输出日志
            if self.current_mode != last_published_mode:
                rospy.loginfo(f"View mode changed to: {self.current_mode} (0=left, 1=right)")
                last_published_mode = self.current_mode
                # 模式变化时多发布几次，确保可靠切换
                for _ in range(3):
                    self.publish_current_mode(force=True)
