...
void MainWindow::initializeROS()
{
    try {
        ...
        vacuum_control_client_ = nh_.serviceClient<servo_wrist::VacuumCmd>("/vacuum_cmd");
...
void MainWindow::sendHomeCommand()
{
    ...
    // 使用服务调用（如果可用）
    ros::ServiceClient homeClient = nh_.serviceClient<servo_wrist::HomePosition>("/home_position");
...
cv::Mat MainWindow::createColoredDepthMap(const cv::Mat& depth_map)
{
    // 检查输入
