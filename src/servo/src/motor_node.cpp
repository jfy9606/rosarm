...
MotorNode::MotorNode(const std::string& port, int baudrate) 
    ...
    if (!initializeSerialPort(port, baudrate)) {
        ROS_ERROR("Initialization of serial port failed, will attempt to reconnect periodically");
        // 设置定期重试连接的定时器
    ...
    // 设置服务
    motor_control_service_ = nh_.advertiseService("motor_control", &MotorNode::motorControlServiceCallback, this);
...
std::string MotorNode::findSerialDevice(const std::string& preferred_port) {
    ...
    // 如果没有找到设备，返回原始端口
    ROS_ERROR("No available serial device found, will attempt to use original port: %s", preferred_port.c_str());
    return preferred_port;
}
...
void MotorNode::motorOrderCallback(const servo::MotorOrder::ConstPtr& msg) {
...
bool MotorNode::motorControlServiceCallback(servo::MotorControl::Request& req, 
                                         servo::MotorControl::Response& res) {
    // 处理电机控制服务请求
    bool success = sendMotorCommand(req.motor_id, 0, req.velocity, req.acceleration, 
                                   req.deceleration, req.is_position_mode,
                                   req.position, req.position_threshold);
    
    res.success = success;
    res.message = success ? "Successfully controlled motor" : "Motor control failed";
...
bool MotorNode::sendMotorCommand(uint8_t motor_id, uint8_t form, int16_t velocity,
    ...
    // 检查串口连接
    if (serial_port_ < 0) {
        ROS_ERROR("Serial port not opened, attempting to reconnect");
