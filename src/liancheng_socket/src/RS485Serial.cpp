...
serialpublisher::serialpublisher(ros::NodeHandle & nh){
        ...
        // Get port from parameter server or use default
        if (!nh_.getParam("port", port)) {
        port="/dev/ttyUSB0";
            ROS_INFO_STREAM("Using default port: " << port);
        } else {
            ROS_INFO_STREAM("Using port from parameter: " << port);
        }
        ...
        try {
            ...
            ser.setTimeout(to);
            ser.open();
            ROS_INFO_STREAM("Serial port opened: " << port);
        } catch (serial::IOException& e) {
            // If specified port fails, try the other USB port
            std::string alt_port = (port == "/dev/ttyUSB0") ? "/dev/ttyUSB1" : "/dev/ttyUSB0";
            ROS_WARN_STREAM("Could not open " << port << ", trying " << alt_port << " instead");
            try {
                ser.setPort(alt_port);
                ser.setBaudrate(baud);
                ser.setTimeout(to);
                ser.open();
                port = alt_port;
                ...
                ROS_ERROR_STREAM("无法打开串口设备: " << alt_port);
                throw;
            }
        }
...
{
    if (!ser.isOpen()) {
        ROS_WARN_STREAM("Serial port not open. Attempting to open " << port);
        try {
        ser.setPort(port);
        ser.setBaudrate(baud);
        ser.setTimeout(to);
